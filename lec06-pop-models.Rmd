...
- Qualitatively analyze one-dimensional population models by hand and in R
    - Find fixed points graphically
    - Analyze stability graphically
    - Draw one-dimensional phase portraits
...
1. Models can simplify a complex system and help us identify what's important.
  ...
  force us to identify the important parameters and distill them down to a manageable number of parameters
   ...
   *Note*: that's not always how modelling is done. Sometimes you just add everything you can think of 
   ...
   and make better predictions. But to do that you need to simulate the model - it will likely 
  ...
  ![Signal transduction pathway](1280px-Signal_transduction_pathways.png)
  
  [Image source](https://en.wikipedia.org/wiki/Modelling_biological_systems#/media/File:Signal_transduction_pathways.svg)
...
2. Models can show us something we didn't expect, that we couldn't intuit from just thinking about the problem. 
  ...
  models are classic because their results weren't obvious before the model was created. 
    - example: Lorenz model (show video)
    - example: Lotka-volterra predator-prey model (show simulation in R)
    - example: model from my research (show zyx vs pV)
...
plot(head(population_size,-1),N_diff)
lines(head(population_size,-1),N_diff)
```
...
plot(N_seq,dN_dt)
lines(N_seq,0*N_seq) # a line at zero for visual aid
```
