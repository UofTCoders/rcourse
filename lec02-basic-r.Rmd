...
making all our content available under the same license, The Creative Commons, 
license, so if someone in the future are teaching this course content available under
the same lice*

I am emphazing this because I think it is one of the core features of working
with an open language like R, many smart people readily share their material and
discoveries not only for free, but in a way that allows others to modify and
built on the material themselves. It really is "standing on the shoulders of
giants", and there is a common saying in the open source world:

> I came for the language and stayed for the community.

Which highlights the spirit, generousity, and the fun involved in many of these project.

------------


## Recap from intro lecture

We will be working with the programming language R through an interface called
RStudio. We will mainly be using the R Notebook, which allows us to mix R code
with writing notes in the Markdown text format. So let's open an R Notebook in
RStudio... You will be prompted to install a few packages, click Yes/OK. I have
asked the IT services to make sure these packages are always installed on the
computers so that we don't have to install them every time, hopefully they will
do so soon. If you prefer, you can also bring your own laptop to class, which
has the advantage that you can save things permanently and that you have
everything setup, this is up to you <!--ok to say this?-->. Let's go over the
lesson objectives while we wait for these packages to install. 
...
Let's remove the template R Studio gives us, and add a title of our own.
...
This introductory block is call the `yaml`-block and is where we specify things
like whether we want html or PDF output, but more on that another class. For
now, we just care about including the lecture here. Under it let's type a note:
...
chunk by either clicking this button or pressing <kbd>Ctrl</kbd> +
<kbd>Alt</kbd> + <kbd>i</kbd>. You could also type out the surrounding
delimiters, but this would take longer. To run a code chunk, you press this
green arrow, or <kbd>Ctrl</kbd> + <kbd>Shift</kbd> + <kbd>Enter</kbd>.
...
_objects/variables_. To create an object, we need to give it a name followed by
the assignment operator `<-`, and the value we want to assign:
...
What are known as `objects` in `R` are called `variables` in many other
programming languages.  Depending on the context, `object` and `variable` can
have drastically different meanings.  However, in this lesson, the two words are
used synonymously and in this introductory section, the title "Creating object
in R" is the same as "Assigning a value to a variable" (for more information
see: https://cran.r-project.org/doc/manuals/r-release/R-lang.html#Objects).
...
used because they are the names of fundamental functions in R (e.g., `if`,
`else`, `for`, see
[here](https://stat.ethz.ch/R-manual/R-devel/library/base/html/Reserved.html)
for a complete list). In general, even if it's allowed, it's best to not use
other function names (e.g., `c`, `T`, `mean`, `data`, `df`, `weights`). If in
doubt, check the help or use tab completion to see if the name is already in
use. It's also best to avoid dots (`.`) within a variable name as in
`my.dataset`. There are many functions in R with dots in their names for
historical reasons, but because dots have a special meaning in R (for methods)
and other programming languages, it's safest to avoid them and use underscores
(`_`) instead. It is also recommended to use nouns for variable names, and verbs
...
[**`lintr`**](https://github.com/jimhester/lintr) to automatically check for
issues in the styling of your code.
...
The variable `weight_kg` is not stored in the computer's memory where R can
access it, and we can start doing arithmetic with it efficiently. For instance,
we may want to convert this weight into pounds (weight in pounds is 2.2 times
the weight in kg):
...
Traditionally, comments has been the only way to make notes when you are coding,
but since we are using the R Notebook format, we can also write notes in
...
even a dataset. We'll see that when we read data files into R, as I showed you
briefly in during the first class.
...
means that it is a variable that we defined earlier. There are a few other
symbols as well, all of which can be viewed at the end of [this post about
RStudio code
...
will talk more about that later. But briefly, a vector is a set of one or
several numbers, and in R, every value is a vector even if it only is a single
number. So, you don't have to do anything special in R to create a vector, but
more on that later.
...
We can leave actually, out the word `digits` since we know it comes as the
...
are using a function that has be made previously and built into R. To remove
some of the magic around these functions, we wil go through how to make a basic
...
functions if there is no vectorized way of going about what you want to to.

You can think of a for-loop as: "for each number contained in a list/vector,
perform this operations" and the syntax basically says the same thing:
...
# don't retain the value from last time we ran this code.
w <- 0
for (num in v) {
    w <- w + num
}
...
ready made function that are much more efficient in working with vectors and
essentially essentially performs operations on entire vector at once instead of
one number at a time. If anyone is interested in more details about how this
work, please ask me after class, but conceptually loops operate on one element
at a time while vectorized code operate on all elements of a vector at once.

*Go to the output options and set it to `html_document`, and then knit*

**Remember to save your notebook file (.Rmd) on a USB stick or email it to
yourself**
