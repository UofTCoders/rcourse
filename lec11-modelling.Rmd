...
Theoretical models are powerful tools at explaining or understanding the world.
However, they are limited in that the real-world often doesn't perfectly fit
these models. The real world is messy and noisy. We can't always trust our data
...
Where each $x$ is a variable (either continuous or categorical) in a data frame.
...
measure, so we want to derive a formula in order to predict the variable based
...
non-existant?? No. But math doesn't care what is physically possible or not.
...
constants and coefficients, but it is also used to calculate how *certain* we
...
(counterintuitively, a larger confidence interval is more strict).
...
Great! So in almost all cases, you should favour confidence intervals and
...
![Visualizing raw data vs aggregate](boxplot-problem.gif)
