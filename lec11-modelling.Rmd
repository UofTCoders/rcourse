...
"Dipodomys" is equal to "Yes" and if not is "No" (hint: use `ifelse`). Then,
create a model called `fit_challenge1` that has the terms `sex`,
`hfl_mean_center`, and `Dipodomys`. Extract the relevant information from the
model using `tidy`. What is the weight when sex is male, hindfoot length is 5
above the mean, and Dipodomys is yes?
...
model_challenge1 <- glm(weight ~ sex + hfl_mean_center + Dipodomys, data = challenge1)
tidy(model_challenge1)
```
