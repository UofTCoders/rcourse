...
library(tidyverse)
plant_data <- read_csv("data/Santangelo_JEB_2018.csv", 
                              col_names = TRUE)
glimpse(plant_data)
head(plant_data)
```
...
(b) Plot the distribution of sample means obtained by generating 1000 gamma distributions with the same parameters as in (a). In other words, the distribution should be made up of 1000 means, each from a different simulated gamma distribution. What do you notice about this distribution when compared to the original distribution in (a)? Why would we expect this? (0.5 marks)
...
for(i in 1:nreps){
    ...
    median_grp1 <- median(reshuffled %>% filter(group == "One") %>% pull(x))
    median_grp2 <- median(reshuffled %>% filter(group == "Two") %>% pull(x))
