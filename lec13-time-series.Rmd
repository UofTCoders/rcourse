...
outlined last week. But there are situations when using `lm` or `glm` doesn't
make as much sense, especially when you violate the assumptions they rely on.
...
In order to use the date as a numeric value for fitting purposes, 
...
Not bad! To wrap up, let's fit the rest of the individuals and look at the fits for each species.
...
```{r}
which(resids_sq_individuals$val.resids_sq == min(resids_sq_individuals$val.resids_sq))
...
ggplot(b_df_all, aes(x = scientificName, y = phase, colour = scientificName)) +
  ...
  #geom_point(data = b_df, aes(x = scientificName, y = fit, colour = scientificName))
```
