...
changes in any files, and is an indispensable tool for collaborative projects.

Git is often used via [GitHub](http://www.github.com), a website that
hosts Git repositories. GitHub therefore facilitates collaboration
by allowing for code review and other useful features. In the spirit of 
'working open' and ensuring scientific reproducibility, it has also become increasingly 
...
The members of group will be given admin access to a pre-made repository. 

### Command line Git

#### Setup and Installation

Git is primarily used through a _bash interpreter_, also known as a 'bash shell', 
'the command line', or 'the terminal'. While bash interpreters are 
natively available on Mac and Linux operating systems, Windows users may 
have to externally install a bash shell.
...
`mkdir`, or 'make directory', will create a few directory with
...
`nano` is a text editor, similar to Notepad on Windows, except that
it opens right within the bash window. If the filename provided doesn't 
...
*Material for the following section of the class was taught from [Software Carpentry](www.software-carpentry.org)'s openly available Intro to Git lesson,
...
But what if we do want our changes to show on the main repo? This is
a two step process:

1. First, we create a _branch_ on our _fork_ (not the main repo). This is yet 
another copy of the fork (a copy of a copy!) that will store changes separately 
from 'master', which is the default branch. 

2. Once we have made our changes in this branch, we will submit what's
...
is reviewed by more than one person. Understanding how and why this process works
...
We can now use the dropdown to switch back to `master`, but let's stay
...
After we've made our commit, GitHub takes us back to the main repo
page, but now with an attention-grabbing yellow prompt about our
new edit. GitHub has noticed that we have a new branch with
edits, and over on the right side of the prompt, we have the option
...
Either way, once we decide to open a pull request, GitHub takes us to
the 'Pull requests' tab of the _main repo_ and prompts us to write about
our pull request. Here, we can (and should) explain the changes we've
...
('adding data cleaning `dplyr` code') followed by a point-form list of
...
If all does not look good, your team members can add messages below, and
...
Once a pull request has been merged, the branch on our fork isn't needed
anymore. Because of this, GitHub will immediately prompt us to delete
the `initial-edit` branch as soon as it's been merged. Our edits
are now on the main repo!

#### Syncing your fork

There's one issue with the above, though -- now, the `master` branch
on our fork is out of date with the current state of the main repo.
To update our fork, we have to go back to the command line.

To begin with, _clone_ your fork to your computer. To do this,
open a bash shell, and navigate over to whichever folder you would
...
This will create a new folder in your current working directory that contains
the contents of your fork. Enter this new folder with `cd` and type `git status`
to make sure the repo has been cloned properly. `git status` should output
that the branch is even with `origin/master`, indicating that it is currently
the same as the current state of your fork. 

To get our fork up to date with the main repo, we have to add a _remote_ 
...
we have to push these changes to the GitHub version of our fork:
...
would otherwise clash with someone else's if one of you is working 
with an earlier version of the file.
...
5. Have your group members review your pull request, and merge if appropriate
6. Delete your branch
7. Sync your fork to be up to date with the new changes
