...
p1 <- ggplot() + # assign plot to a variable so we can use it later
  geom_path(aes(x = X, y = Y), result) + # trajectory
...
$r$ is a number we haven't chosen, but we do know that it's always positive. That means $N = 0$ is an unstable fixed point. 
...
### Two dimensions: calculating the Jacobian matrix

Analyzing stability in two dimensions follows exactly the same principle, but the math is a little bit more complicated. Now instead of needing to find the slope in just one dimension, we have an two-dimensional surface we need to consider. 
