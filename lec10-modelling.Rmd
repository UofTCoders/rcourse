...
> - Vectorizing linear systems of differential equations: another perspective on eigenvalues
> - Bifurcations
> - Fitting models to data
>     - Fitting simulated data for a single parameters
>     - Fitting real data for two parameters
...
at each fixed point, where $f(N) = rN(1-\frac{N}{K}) - HN$.

For the fixed point $N=0$:

$$\frac{\partial f(N)}{\partial N}$$
...
- simulate data from exponential model and do least squares


Suppose we sample repeatedly from a population over time, and suppose we want to fit our sampled data to a logistic model.

Let's simulate some (error-free) data from an exponential function to work with.
...
Ndata <- K*exp(r*times)/(exp(r*times)+C)

plot(times,Ndata) # check with a plot
```



- fit plant biomass data
...
data <- read.csv("plant-biomass-preprocess.csv")
```
