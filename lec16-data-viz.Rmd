...
It is similar to a powerful ToDo manager for your project. In general, you
...
*Create new issue about adding a file to the sandbox repo*
...
3. In RStudio, open a new git project.
    - `File -> Version Control -> Git`. Paste the URL you copied previously and select where to save the directory.
...
5. Open a terminal and navigate to the new directory.
    - You could do the navigation in your file browser and then right click to
      open a terminal
        - Right click should be enabled by default with Git.. on Windows. For
          Mac's Finder, follow [these
...
7. Before we add something to this repository, it is good practice to create a
   new branch instead of working on the default master branch. Branches allows
   you to work on features separately and can make it clearer what is what when
   merging your code with others.
    - This is the only step that *needs* to be done from the command line.
      Navigate to the directory just created via RStudio and type `git branch
      <branchname>`. You can now switch to the new branch in the RStudio
      interface (you could also type `git checkout -b <branchname>` to create
      and switch to a branch in one step in terminal).
...
12. Now we can commit our changes, which means saving them into our local git
   repository. Add a commit message and click `Commit`.
    ...
    - The reason you are staging first and then committing, instead of just
      "saving" in one step, is that this gives workflow is more powerful and
      more flexible. For example, it allows you to stage partial changes from a
      file or changes from several file and then commit them together as one
