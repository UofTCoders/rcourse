...
But we can also solve `r eq_dn_dt` analytically to find the fixed points. 
...
points we found graphically last week. 
...
```{r}
ggplot(result) +
  geom_point(aes(x = X, y = Y)) # geom_line connects the dots strangely
```
...
saveGIF({ 
    par(mfrow=c(2,1))
    for (i in length(result$time)){ 
      ggplot(result) +
        geom_line(aes(x = result$time[1:i], y = result$X[1:i]), color = 'red') +
        geom_line(aes(x = result$time[1:i], y = result$Y[1:i]), color = 'blue') 

      ggplot(result) +
        geom_point(aes(x = result$X[1:i], y = result$Y[1:i]))
    }
...
INSERT GIF from school computer

### Drawing qualitative phase portraits by hand

Before we look at how to add more to phase portraits in R, let's pratice the process by hand to
...
We want to draw a **vector field** in two dimensions to show how the system will evolve in time.
...
## Fixed points and null clines in two-dimensional systems
...
points(4, 1, # x, y coordinates of point to plot
