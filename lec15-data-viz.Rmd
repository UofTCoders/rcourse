...
[http://ggplot2.tidyverse.org/reference/theme.html](This is a resource on which parts of the plot are controlled by different theme parameters when and if you create your own theme.)
...
```{r}
library(plotly)
g <- ggplot(iris, aes(Species)) + geom_bar()
p <- ggplotly(g)
p
```
