...
## Recap and some tips
   ...
   - **Differential equation**: an equation that describes how a function of variables is related to
derivatives of those variables. 
   ...
   - Drawing **phase portraits** in one dimension:
    ...
    ![Logistic equation simulation](https://github.com/UofTCoders/rcourse/raw/master/logistic.gif)
       ...
       - Fixed points: values of $N$ at which $dN/dt$, the rate of change of $N$, is $0$. 
       To find fixed points, plot $dN/dt$ vs. $N$ and find the place(s) where it crosses the $x$ axis. 
       - Stability: if you start at some $N$ close to the fixed point but not exactly on it, 
       will you go towards (stable) or away (unstable) from the fixed point? The sign of $dN/dt$ on
       either side of a fixed point tells you whether $N$ will increase or decrease in that area. 
       Draw an arrow to the right if $dN/dt$ is positive, and draw and arrow to the left if $dN/dt$ is negative. 
...
I'm moving this topic ahead so that we can use what we learn here to aid us in analyzing more
complicated models in one and two dimensions. 
...
other methods, but you can look up *Runge-Kutta* methods if you're interested in learning more about it. 
...
$$N_1 =  2+ 1.6 = 3.6$$
We repeat the proccess to get $N_2$, and so on, remembering to evaluate $dN/dt$ at the value of $N$ from the previous timestep. The vertical bar $\vert$ instructs us to substitute $N_0$ for $N$ in the derivative. 
...
```{r}
print(head(result,10)) # result has a list of times and values of N at those times
print(typeof(result)) # result is a 'double': a vector of numbers.
```
...
lotka_volterra <- function(t,y,parameters) { 
    ...
    X <- y[1]; Y <- y[2] # now the state vector y has two elements because we have two species
