...
derivatives of those variables. 
   ...
   - Drawing **phase portraits** in one dimension:
    ...
    ![Logistic equation simulation](https://github.com/UofTCoders/rcourse/raw/master/logistic.gif)
       ...
       Plot $dN/dt$ vs. $N$ and find the place(s) where it crosses the $x$ axis. 
       - Stability: if you start at some $N$ close to the fixed point but not exactly on it, will you go towards (stable) or away (unstable) from the fixed point? The sign of $dN/dt$ on either side of a fixed point tells you whether $N$ will increase or decrease in that area. Draw an arrow to the right if $dN/dt$ is positive, and draw and arrow to the left if $dN/dt$ is negative. 
